...
class AddTagsToProjects extends Component {
  constructor(props) {
    super(props);
  ...
  shouldComponentUpdate = (nextProps, nextState) => {
    if (this.props.details !== nextProps.details) {
      this.filterProjects(nextProps.details, nextProps.id);
      return true;
    }
    return false;
  };
  ...
  filterProjects = (serviceDetails, serviceId) => {
    if (this.state.dbref) {
      const p_ref = db.ref("/" + this.state.dbref);
...
export default AddTagsToProjects;
